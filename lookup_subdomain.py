# -*- coding:utf-8 -*-
# just for learning, origins from https://github.com/ghostflyII/lookup_subdomain
...
def baidu_subdomains(url,UA):
    subdomains=[] #先创建一个存放子域名的数组
    request=urllib2.Request(url='http://ce.baidu.com/index/getRelatedSites?site_address=%s' % url) #调用百度接口
    request.add_header('User-Agent',UA) #request包里加上user-agent
    ret=urllib2.urlopen(request) #请求
    try:
        print datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')+u' 正在通过百度接口进行查询......'
        ret=json.loads(ret.read())  #返回接口查询结果
        #print ret
    except Exception, urllib2.HTTPError: #如果请求错误
        print datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')+u' 警告：百度接口连接错误 ' + str(urllib2.HTTPError)
        return subdomains
    else:
        if ret.get('code')==0: #如果code=0 正常返回数据
            for data in ret.get('data'):
                subdomains.append(data.get('domain')) #把接口返回的子域名信息加入subdomains数组中
            print datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')+u' 百度接口查询结果返回中......'
        else:
            print datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')+u' 警告:'+ret.get('message') #如果code不等于0 则直接返回
        return subdomains

def virustotal_subdomains(url,UA):
    subdomains = []
    request = urllib2.Request(url="https://www.virustotal.com/ui/domains/%s/subdomains"%url) #同上的过程只是调用的是virustotal的接口
